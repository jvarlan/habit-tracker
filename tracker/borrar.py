...
def borrar_objetivo(borrar,seleccion):
    ...
    if len(filas_originales) == len(filas_restantes):
        return (f"{ROJO}El registro  no existe.{RESET}")
    else:
        return (f"{VERDE}Registro  eliminado.{RESET}")
    
def borrar_objetivos_id_habito(borrar):
    ruta = BASE_DIR / "datos" / "objetivos.csv"
...
def borrar_categoria(borrar,id_categoria,lista_habitos,lista_temporizadores, lista_objetivos_categoria):
    ...
    for objetivos in lista_objetivos_categoria:
        print(objetivos[0])
        borrar_objetivo(objetivos[0],objetivos[1])
